...
offers some boosts in performance for the plugin loader.
 ...
 1. The filename contains a version string: `supermodules-v1.2.3.mmplugin`.
    This MUST be in the form `-vX.X.X` where X is a series of digits 0-9.
 2. Inside the `.mmplugin` tar bundle, there is a file called `SDK-X.X` which
...
and memory, the version is embedded into the filename so that the plugin loader
GUI can quickly display all found plugin files.
